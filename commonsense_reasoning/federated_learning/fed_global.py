...
def get_clients_this_round(args, round):
    if (args.fed_alg).startswith('local'):
        clients_this_round = [int((args.fed_alg)[-1])]
    else:
        if args.num_clients < args.sample_clients:
            clients_this_round = list(range(args.num_clients))
        else:
            random.seed(round)
            clients_this_round = sorted(random.sample(range(args.num_clients), args.sample_clients))
    return clients_this_round

def global_aggregate(args, global_dict, local_dict_list, sample_num_list, clients_this_round, round_idx, proxy_dict=None, opt_proxy_dict=None, auxiliary_info=None):
    ...
    if args.fed_alg == 'scaffold':
        ...
        for key in global_auxiliary.keys():
            ...
            global_auxiliary[key] += delta_auxiliary / args.num_clients
