        ...
        for key, value in local_dict.items():
            ...
            if "lora_A" in key:
                ...
                if lora_B_key in local_dict:
                    ...
                    if lora_A.shape[0] != lora_B.shape[1]:
                        max_dim = max(lora_A.shape[0], lora_B.shape[1])
                        lora_B = torch.nn.functional.pad(lora_B, (0, max_dim - lora_B.shape[1]), "constant", 0)
                        lora_A = torch.nn.functional.pad(lora_A, (0, 0, 0, max_dim - lora_A.shape[0]), "constant", 0)
