...
def partition(args, train_dataset, test_dataset):
    ...
    if args.data_partition_method == 'iid':
        ...
        for i, subset in enumerate(subsets):
            ...
            train_dataloader_list[i] = DataLoader(subset, sampler=train_sampler, batch_size=args.micro_batch_size)
        ...
        #切分测试集
        # subset_size = len(test_dataset) // args.num_clients
        # remaining_size = len(test_dataset) - subset_size * args.num_clients
        # subset_sizes = [subset_size] * args.num_clients
        # for i in range(remaining_size):
        #     subset_sizes[i] += 1
        # subsets = random_split(test_dataset, subset_sizes)
        # print('number of samples for test')
        # for i, subset in enumerate(subsets):
        #     test_sampler = SequentialSampler(subset)
        #     test_dataloader_list[i]=DataLoader(subset, sampler=test_sampler, batch_size=args.micro_batch_size)
        #     print(f'Client {i}: {len(test_dataloader_list[i].dataset)}')

        #不切分测试集
        for i in range(args.num_clients):
            test_sampler = SequentialSampler(test_dataset)
            test_dataloader_list[i]=DataLoader(test_dataset, sampler=test_sampler, batch_size=args.micro_batch_size)
            print(f'Client {i}: {len(test_dataloader_list[i].dataset)}')
        ...
        for i, subset in enumerate(subsets):
            train_sampler = RandomSampler(subset)
            train_dataloader_list[i] = DataLoader(subset, sampler=train_sampler, batch_size=args.micro_batch_size, collate_fn=train_dataset.collate_fn)
            print(f'Client {i}: {len(train_dataloader_list[i].dataset)}')
            n_sample_list[i] = len(train_dataloader_list[i].dataset)
        ...
        if args.load_from_cache and (os.path.exists(os.path.join(args.data_dir, 'saved_train_subsets.pkl'))) \
            ...
            for client_idx in range(args.num_clients):
                ...
                train_sampler = RandomSampler(subset)
                train_dataloader_list[client_idx] = DataLoader(subset, sampler=train_sampler, batch_size=args.micro_batch_size)
                
            for client_idx in range(args.num_clients):
                subset = test_subsets_list[client_idx]
                test_sampler = SequentialSampler(subset)
                test_dataloader_list[client_idx] = DataLoader(subset, sampler=test_sampler, batch_size=args.micro_batch_size)
...
def partition_gpt2(args, train_dataset, test_dataset):
    train_dataloader_list = [copy.deepcopy(1) for _ in range(args.num_clients)]
    test_dataloader_list = [copy.deepcopy(1) for _ in range(args.num_clients)]
    ...
    if args.data_partition_method == 'iid':
        ...
        for i, subset in enumerate(subsets):
            test_sampler = SequentialSampler(subset)
            test_dataloader_list.append(DataLoader(subset, sampler=test_sampler, batch_size=args.micro_batch_size, collate_fn=test_dataset.collate_fn))
            print(f'Client {i}: {len(test_dataloader_list[i].dataset)}')
        ...
        if args.load_from_cache and (os.path.exists(os.path.join(args.data_dir, 'saved_train_subsets.pkl'))) \
            ...
            if args.personalization:
                ...
                for client_idx in range(num_clients):
                    ...
                    subset_list.append(subset)
                    test_sampler = SequentialSampler(subset)
                    test_dataloader_list[0][client_idx] = DataLoader(subset, sampler=test_sampler, batch_size=args.micro_batch_size)
                subset_list = []
                for client_idx in range(num_clients):
                    subset = Subset(test_dataset[1], sample_idx_per_client[client_idx])
                    subset_list.append(subset)
                    test_sampler = SequentialSampler(subset)
                    test_dataloader_list[1][client_idx] = DataLoader(subset, sampler=test_sampler, batch_size=args.micro_batch_size)
